...
class PipelineExecute:
    def __init__(self):
        ...
        self.RDQueue = []   #Queue to store RD(s) from all the instruction from proper pipeling
        self.branchTaken = False
        self.total_ins = 0  #For counting purposes
        self.total_control_ins = 0  #For counting purposes
        self.total_data_ins = 0 #For counting purposes
        self.total_alu_ins = 0  #For counting purposes
        self.data_hazards = 0   #For counting purposes
        self.control_hazards = 0    #For counting purposes
        self.was_data_hazard = False    #For counting purposes
        self.printRegisterFileKnob = False
        self.printRegisterPipelineKnob = False
        self.knob5 = False
    ...
    def runPipeLineStep(self):
        ...
        if self.memAccess_run:
            #print('---------------------------')
            self.writeReg() #4th stage
            #print('---------------------------')
        if self.alu_run:
            #print('---------------------------')
            self.memAccess()    #3rd stage
    ...
    def decode(self):
        ...
        try:
            ...
            self.preload = False
        self.decode_buffer = {'mem_enable' : '-1', 'RD' : '-1', 'muxY' : 0, 'load' : False} #Decode buffer to store previous instruction gone through this stage
        self.opcode = self.IR[25:32]
        #print("opcode:"+self.opcode)
        self.RB = 0
        format = self.checkFormat()
    ...
    def alu(self):
        ...
        if operation == "add":
            ...
            if self.RA == self.RB:
                ...
                self.fetch_run = False  #stalling whole cycle for hazard control
    ...
    def decodeR(self):
        ...
        self.RB = self.RegisterFile.readB(self.RS2)
        #print("RB:"+str(self.RB))
        if self.RS1 in self.RDQueue or self.RS2 in self.RDQueue:    #checking if rs1 and rs2 are in being used as rd for some other instruction
            if self.do_dataForwarding:
                reversedQueue = list(reversed(self.RDQueue))    #Reversing for last instruction check because if the rs(s) are not being used in last instruction then data forwarding may not be needed
                ...
                self.was_data_hazard = True
                self.decode_run = False #stops next stage to run for that instructions hence stalling
                self.PC = self.PC - 4
                #print('Stalling!')
                self.stalls_data = self.stalls_data + 1
                return
        if self.was_data_hazard:
            ...
            self.was_data_hazard = False
        self.decode_buffer['RD'] = RD   #updating decode buffer for hazard manipulation
        self.RDQueue.append(RD) #updating RD in queue for future data forwarding
        self.muxB = 0
        self.funct3 = self.IR[17:20]
        self.funct7 = self.IR[0:7]
        #self.muxY=0
        self.total_alu_ins = self.total_alu_ins + 1 #instruction count update
        if self.funct3 == "000" and self.funct7 == "0000000":
